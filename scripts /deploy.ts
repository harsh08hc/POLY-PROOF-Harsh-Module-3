...
const BASE_PATH = "./circuits/CircuitAssessment/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/CircuitAssessment.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/CircuitAssessmentVerifier.sol:Verifier");

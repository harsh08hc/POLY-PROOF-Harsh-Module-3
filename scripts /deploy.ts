...
const BASE_PATH = "./circuits/harshcircuit/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/harshcircuit.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/HarshcircuitVerifier.sol:Verifier");
